...
import time
from config import BASE_DIR  # Assuming config.py is available in the correct location
...
class SkinDiseaseModel:
    def __init__(self, model_type="mobilenetv2"):
        ...
        self.create_label_encoder(data_path=BASE_DIR / "dataset/train_set")
    ...
    def train_model(self, data_path, batch_size=2, epochs=12, override=False):
        ...
        try:
            ...
            plt.plot(
                range(1, epochs + 1), train_acc, label="Training accuracy", marker="o"
            )
            plt.title("Training Accuracy")
            plt.xlabel("Epochs")
            plt.ylabel("Accuracy")
            plt.legend()
            plt.savefig(f"training_accuracy_{self.model_type}.png")
...
if __name__ == "__main__":
    ...
    data_directory = BASE_DIR / "dataset/train_set"
    test_directory = BASE_DIR / "dataset/test_set"
    example_image_path = BASE_DIR / "img/FU-athlete-foot_27.jpg"
    example_image_path = example_image_path.as_posix()

    # Uncomment the line below to train the model
    model_instance.train_model(data_directory, override=True)

    # Evaluate the model on the test set
    evaluate_result = model_instance.evaluate_model(test_directory)
    print(evaluate_result)

    # Predict the class for an example image
    predict_result = model_instance.predict_class(example_image_path, num_predictions=3)
